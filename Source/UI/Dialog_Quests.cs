...
namespace RimCoopMod.UI
{
    ...
    public class Dialog_ShareQuest : Window
    {
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            Widgets.Label(new Rect(0f, 0f, inRect.width, 34f), Loc.T("Dialog_Quests.01", _playerName));
            Text.Font = GameFont.Small;
            Widgets.Label(new Rect(0f, 36f, inRect.width, 60f),
                Loc.T("Dialog_Quests.08", CoopSessionManager.QuestDifficultyPerPlayer));
            ...
            if (quests.Count == 0) Widgets.Label(new Rect(0f, 0f, viewRect.width, 30f), Loc.T("Dialog_Quests.02"));
            float y = 0f;
            foreach (var q in quests)
            {
                Widgets.Label(new Rect(0f, y + 6f, viewRect.width - 120f, 30f), q.name);
                if (Widgets.ButtonText(new Rect(viewRect.width - 110f, y, 110f, 34f), Loc.T("Dialog_Quests.03")))
                {
    ...
    public class Dialog_QuestInvite : Window
    {
        ...
        {
            Text.Font = GameFont.Medium;
            Widgets.Label(new Rect(0f, 0f, inRect.width, 34f), Loc.T("Dialog_Quests.04", _invite.FromPlayerName));
            Text.Font = GameFont.Small;
            ...
            Widgets.Label(new Rect(0f, 62f, inRect.width, 48f),
                Loc.T("Dialog_Quests.09", mult, CoopSessionManager.QuestDifficultyPerPlayer));
            ...
            if (Widgets.ButtonText(new Rect(0f, inRect.height - 40f, third - 6f, 36f), Loc.T("Dialog_Quests.05")))
            {
            ...
            if (Widgets.ButtonText(new Rect(third + 3f, inRect.height - 40f, third - 6f, 36f), Loc.T("Dialog_Quests.06")))
            {
                Close(); // sigue pendiente: se vuelve a mostrar (también después de guardar y cargar) cuando el otro esté conectado
            }
            if (Widgets.ButtonText(new Rect(third * 2f + 6f, inRect.height - 40f, third - 6f, 36f), Loc.T("Dialog_Quests.07")))
            {
