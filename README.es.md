...
El mod está completo en **español e inglés**. Por defecto usa el idioma del juego; se cambia en *Opciones → Ajustes de mods →
RimCoop* o con el botón **Idioma** de las ventanas de conexión y de servidores (se aplica al instante). El servidor
también: `--idioma es|en` al abrirlo, o el comando `idioma es|en` (queda guardado). Cada jugador ve los textos en su propio
idioma, aunque los demás jueguen en otro.
...
- **Comandos de la consola** (escribí `ayuda`): `jugadores` (quién está conectado), `idioma es|en`, `kick <nombre|id> [motivo]` (lo saca, puede
